...
If a particle were to collide with each of the $e$ edges in the workpiece once, there would be $e^2$ total intersection tests for a single particle simulation.
...
Note that Equation \eqref{eq:bigo} provides a meaningful upper-bound on the number of intersection tests that must be performed with one core assumption -- each particle during simulation intersects with each edge of the workpiece once.
